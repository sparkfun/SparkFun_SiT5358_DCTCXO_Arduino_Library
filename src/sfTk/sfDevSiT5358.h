...
class sfDevSiT5358
{
  public:
    ...
    sfDevSiT5358() : _baseFrequencyHz{10000000.0}, _maxFrequencyChangePPB{3200000.0}
    {
    }

    /// @brief Begin communication with the SiT5358. Read the registers.
    /// @return ksfTkErrOk on success, ksfTkErrFail on error
    sfTkError_t begin(sfTkII2C *commBus);

    /// @brief Read all three SiT5358 registers and update the driver's internal copies
    /// @return true if the read is successful and the Frequency Control MSW and Pull Range Control have 0's where
    /// expected
